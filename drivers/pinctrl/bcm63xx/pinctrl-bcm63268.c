...
#define BASEMODE_UTOPIA			BIT(3) /* none? */
...
#define BASEMODE_VDSL_LED		BIT(10) /* none? */
#define BASEMODE_USB_LED		BIT(11) /* none? */

/* MISC_MISC register */
#define MISC_DSL_GPIO_8			BIT(2) /* GPIO 8 */
#define MISC_DSL_GPIO_9			BIT(3) /* GPIO 9 */
...
struct bcm63268_pinmux_data {
	u32 basemode;
	u32 miscmisc;
};

#define BCM63268_PINMUX_DATA(a, basemode_val, miscmisc_val)	\
static const struct bcm63268_pinmux_data gpio##a##_data = {	\
	.basemode = (basemode_val),				\
	.miscmisc = (miscmisc_val),				\
}

BCM63268_PINMUX_DATA(2, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(3, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(4, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(5, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(6, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(7, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(8, BASEMODE_DECTPD, MISC_DSL_GPIO_8);
BCM63268_PINMUX_DATA(9, BASEMODE_DECTPD, MISC_DSL_GPIO_9);
BCM63268_PINMUX_DATA(10, BASEMODE_VDSL_PHY_0, 0);
BCM63268_PINMUX_DATA(11, BASEMODE_VDSL_PHY_0, 0);
BCM63268_PINMUX_DATA(12, BASEMODE_VDSL_PHY_1, 0);
BCM63268_PINMUX_DATA(13, BASEMODE_VDSL_PHY_1, 0);
BCM63268_PINMUX_DATA(24, BASEMODE_NAND | BASEMODE_VDSL_PHY_2, 0);
BCM63268_PINMUX_DATA(25, BASEMODE_NAND | BASEMODE_VDSL_PHY_2, 0);
BCM63268_PINMUX_DATA(26, BASEMODE_NAND | BASEMODE_VDSL_PHY_3, 0);
BCM63268_PINMUX_DATA(27, BASEMODE_NAND | BASEMODE_VDSL_PHY_3, 0);
BCM63268_PINMUX_DATA(28, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(29, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(30, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(31, BASEMODE_NAND, 0);
BCM63268_PINMUX_DATA(35, BASEMODE_GPIO35, 0);

#define BCM63268_PIN(a, b)				\
	{						\
		.number = a,				\
		.name = b,				\
		.drv_data = (void *)&(gpio##a##_data)	\
	}
...
static const struct pinctrl_pin_desc bcm63268_pins[] = {
	...
	PINCTRL_PIN(6, "gpio6"),
	PINCTRL_PIN(7, "gpio7"),
	BCM63268_PIN(8, "gpio8"),
	BCM63268_PIN(9, "gpio9"),
	BCM63268_PIN(10, "gpio10"),
	BCM63268_PIN(11, "gpio11"),
	BCM63268_PIN(12, "gpio12"),
	...
	/* currently unknown if they occupy gpios */
	PINCTRL_PIN(61, "utopia"),
	PINCTRL_PIN(62, "vdsl_led"),
	PINCTRL_PIN(63, "usb_led"),
};
...
static unsigned utopia_grp_pins[] = { 61 };
static unsigned vdsl_led_grp_pins[] = { 62 };
static unsigned usb_led_grp_pins[] = { 63 };
...
static struct bcm63268_pingroup bcm63268_groups[] = {
	...
	/* unknown pin, but enabled through same registers */
	BCM63268_GROUP(utopia_grp),
	BCM63268_GROUP(vdsl_led_grp),
	BCM63268_GROUP(usb_led_grp),
};
...
static const char * const gpio35_alt_groups[] = {
	"gpio35_grp",
};
...
static const char * const vdsl_led_groups[] = {
	"vdsl_led_grp",
};

static const char * const usb_led_groups[] = {
	"usb_led_grp",
};

static const char * const dsl_gpio8_groups[] = {
	"gpio8_grp",
};

static const char * const dsl_gpio9_groups[] = {
	"gpio9_grp",
};
...
#define BCM63268_MISCMISC_FUN(n, val)			\
	{						\
		.name = #n,				\
		.groups = n##_groups,			\
		.num_groups = ARRAY_SIZE(n##_groups),	\
		.reg = BCM63268_MISCMISC,		\
		.mask = val,				\
	}
...
static const struct bcm63268_function bcm63268_funcs[] = {
	...
	BCM63268_BASEMODE_FUN(utopia, BASEMODE_UTOPIA),
	BCM63268_BASEMODE_FUN(gpio35_alt, BASEMODE_GPIO35),
	...
	BCM63268_BASEMODE_FUN(vdsl_led, BASEMODE_VDSL_LED),
	BCM63268_BASEMODE_FUN(usb_led, BASEMODE_USB_LED),
	BCM63268_MISCMISC_FUN(dsl_gpio8, MISC_DSL_GPIO_8),
	BCM63268_MISCMISC_FUN(dsl_gpio9, MISC_DSL_GPIO_9),
};
...
static void bcm63268_pinctrl_pin_dbg_show(struct pinctrl_dev *pctldev,
					  struct seq_file *s,
					  unsigned offset)
{
...
static void bcm63268_set_gpio(struct bcm63268_pinctrl *pctl, unsigned pin)
{
	...
	const struct bcm63268_pinmux_data *data = desc->drv_data;
	u32 mask = BIT(pin % 32);

	if (data && data->basemode)
		bcm63268_rmw_mux(pctl, pctl->basemode, data->basemode, 0);
	if (data && data->miscmisc)
		bcm63268_rmw_mux(pctl, pctl->miscmisc, data->miscmisc, 0);
...
#endif
	.pin_dbg_show		= bcm63268_pinctrl_pin_dbg_show,
};
...
static int bcm63268_pinctrl_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	void __iomem *led, *mode, *ctrl, *basemode, *miscmisc;
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "miscmisc");
	miscmisc = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(miscmisc))
		return PTR_ERR(miscmisc);
